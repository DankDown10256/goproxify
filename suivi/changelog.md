...
- **HA — un nœud Raft isolé restait bloqué en `Candidate`, aucun leader jamais élu** : `startElection()` ne devenait leader que depuis les réponses des pairs interrogés ; sans pair configuré (nœud seul) ou tous injoignables, la boucle sur `Peers` ne s'exécutait jamais alors même que le quorum (1 voix, soi-même) était déjà atteint. Conséquence : `LeaderID()` restait vide en permanence, `/ha/status` répondait `leader_id: ""`, et toute écriture forwardée par un follower échouait avec `503 {"error":"no leader available"}` — le HA était inutilisable dès que le nœud devait s'élire seul (dernier survivant après panne des autres nœuds, ou cluster à un seul nœud). Le quorum est désormais vérifié immédiatement après le vote auto-attribué, avant toute sollicitation des pairs. (Core `0.8.1`, Admin `0.23.1`)
