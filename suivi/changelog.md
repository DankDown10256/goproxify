...
- **Labo de tests — TLS/HTTP3** : `tests/lab/scripts/tls.sh` génère un certificat auto-signé EC P-256 pour `*.lab.test`, l'importe dans l'Admin via `POST /api/v1/certs/import`, crée les routes `lab-tls.lab.test` et `lab-h3.lab.test` avec `tls_enabled:true`, puis vérifie la connexion HTTPS, la présence de `Strict-Transport-Security`, l'annonce HTTP/3 (`Alt-Svc`) et le CN du certificat servi. Scénario k6 `tls-smoke` pour la charge HTTPS (2 VUs, 15 s, `insecureSkipVerify`). Les deux routes sont résolues dans les `extra_hosts` des runners k6 et tools. Idempotent : cert et routes déjà présents sont ignorés. Commande : `tests/lab/lab.sh tls` ou `tests/lab/lab.sh load tls-smoke`.
