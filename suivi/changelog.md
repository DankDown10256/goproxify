...
- **⚠ « Core » devient « passerelle » (français) / « Edge » (anglais), y compris les identifiants techniques** : le composant Data Plane (reverse proxy) s'appelait « Core » ; il s'appelle désormais **passerelle** dans l'interface, les docs et les traductions FR, **Edge** en EN/ES/DE, et `edge` partout dans le code. Changements visibles :
  ...
  - **Compatibilité prévue pour la mise à jour d'un déploiement existant** (aucune action côté base ou fichiers) : la base SQLite de l'Admin est migrée au démarrage (colonnes `core_*` → `edge_*`, contraintes `CHECK`, valeurs de rôle et de périmètre, index) ; `core.json`, `core-cache.gpx` et `core-tokens.db` sont renommés au premier démarrage de la passerelle ; les clés `core_*` d'un ancien `agent.json`/`edge.json` sont reprises ; les anciennes variables d'environnement `…CORE…` sont reportées sur `…EDGE…` (avertissement au démarrage) ; `goproxify core` reste accepté (avertissement) ; l'Admin accepte encore `?core=` et `/backups/core` ; `architecture.json` et `node_tokens.json` au rôle `core` sont relus comme `edge`. **À faire à la main** : dans `docker-compose.yml`/Portainer/Helm, renommer le service en `goproxify-edge` et l'image en `…/edge` ; pour conserver les données, soit garder le volume existant (`goproxify_core_data`) en le déclarant sous le nom `goproxify_edge_data` avec `name: goproxify_core_data`, soit le copier ; mettre à jour les scripts appelant l'API avec les champs JSON `core_*`, les dashboards Prometheus, et les clients MCP (noms d'outils/paramètres).
  - **Non rétrocompatible** : champs JSON `core_*` des requêtes/réponses d'API, noms d'outils et de paramètres MCP, métriques `gpx_core_*`, noms de service/conteneur/image.
  - Les entrées de ce changelog antérieures à ce changement gardent le mot « Core » : elles décrivent l'état de l'époque. Migration vérifiée par un test (`internal/admin/db`) sur un schéma hérité, et à la main sur une base créée par la version précédente. (Admin `0.26.0`, Passerelle `0.10.0`, Agent `0.6.0`, Webapp `0.13.0`, Landing `0.3.0`)
