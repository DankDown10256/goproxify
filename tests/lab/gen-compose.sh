  ...
  lab-tools:
    ...
    entrypoint:
      ...
      - |
        ...
        mkdir -p /lab/scripts
        for f in seed attacks chaos tls hosts auth cleanup; do eval "printf %s \"\$$S_$$f\"" > /lab/scripts/$$f.sh; done
        exec sleep infinity
    networks: [goproxify_net]
    environment:
      LAB_ADMIN_URL: http://goproxify-admin:9443
X
emb S_ scripts/seed.sh scripts/attacks.sh scripts/chaos.sh scripts/tls.sh scripts/hosts.sh scripts/auth.sh scripts/cleanup.sh
cat <<'X'
  ...
  lab-k6:
    ...
    entrypoint:
      ...
      - |
        ...
        for f in common smoke moderate saturation baseline spike stress soak mixed tls-smoke hosts; do eval "printf %s \"\$$K_$$f\"" > /scripts/$$f.js; done
        mv /scripts/hosts.js /hosts.sh
        exec sleep infinity
    networks: [goproxify_net]
    environment:
X
emb K_ load/common.js load/smoke.js load/moderate.js load/saturation.js load/baseline.js load/spike.js load/stress.js load/soak.js load/mixed.js load/tls-smoke.js scripts/hosts.sh
echo
echo '  # <<< GENERATED'
}
