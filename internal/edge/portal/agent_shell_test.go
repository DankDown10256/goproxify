...
type mockShellHub struct {
	...
	sent   []edgews.Message
	onSend func(name string, msg edgews.Message)
}
...
func (m *mockShellHub) SendToAgentByName(name string, msg edgews.Message) error {
	m.mu.Lock()
...
func TestShellBrokerOpenReady(t *testing.T) {
	...
	hub.onSend = func(_ string, msg edgews.Message) {
		if msg.Type != edgews.TypeShellOpen {
			return
		}
		var p edgews.ShellOpenPayload
		_ = json.Unmarshal(msg.Payload, &p)
		ready, _ := edgews.NewMessage(0, edgews.TypeShellReady, edgews.ShellReadyPayload{SessionID: p.SessionID})
...
func TestShellBrokerOpenError(t *testing.T) {
	...
	hub.onSend = func(_ string, msg edgews.Message) {
		...
		var p edgews.ShellOpenPayload
		_ = json.Unmarshal(msg.Payload, &p)
		errMsg, _ := edgews.NewMessage(0, edgews.TypeShellError, edgews.ShellErrorPayload{
...
func TestShellBrokerRoundTripData(t *testing.T) {
	...
	hub.onSend = func(_ string, msg edgews.Message) {
		...
		case edgews.TypeShellData:
			...
			_ = json.Unmarshal(msg.Payload, &p)
			// echo back
			echo, _ := edgews.NewMessage(0, edgews.TypeShellData, p)
