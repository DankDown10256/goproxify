...
func TestApplyPageTemplateVars(t *testing.T) {
	out := ApplyPageTemplate(
		`<h1>{{brand}}</h1><p>{{user_email}}</p><b>{{ edge_name }}</b><div>{{content}}</div><i>{{csrf}}</i>`,
		PageVars{Brand: "GPX", UserEmail: "a@b.c", EdgeName: "edge1", CSRF: "tok", Content: "<em>ok</em>"},
	)
	for _, want := range []string{"GPX", "a@b.c", "edge1", "<em>ok</em>", "tok"} {
