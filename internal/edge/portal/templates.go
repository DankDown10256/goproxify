...
// ApplyPageTemplate remplace {{brand}}, {{user_email}}, {{edge_name}}, {{csrf}}, {{content}}.
func ApplyPageTemplate(body string, v PageVars) string {
	repl := map[string]string{
		...
		"edge_name":  html.EscapeString(v.EdgeName),
...
func DefaultPageFallback(key string) string {
	...
	case PageLogin:
		return `<section data-page="login"><h1>{{brand}}</h1><p>Connexion Access — {{edge_name}}</p></section>`
