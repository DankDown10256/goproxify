...
// Package cluster gère le groupe de passerelles : membership, leader Raft, synchronisation.
package cluster
...
// Group représente un groupe de passerelles partageant un cluster Raft.
type Group struct {
	name      string
	node      *raft.Node
	raftSrv   *raft.Server
...
// Config configure le groupe de passerelles.
type Config struct {
	NodeID    string
	GroupName string
	// Peers: map id → base URL (ex: "edge-2" → "http://10.0.0.2:8002")
