...
func TestPeerRegistryReplaceAndLookup(t *testing.T) {
	r := NewPeerRegistry()
	r.Replace([]PeerInfo{
		{Name: "edge-b", Endpoint: "http://10.0.0.2:8000/", Token: "tok-b"},
	})
...
func TestMergeRemoteBackends(t *testing.T) {
	// Exercice via logique équivalente (copie du helper edge)
	existing := []router.Backend{
		{URL: "http://10.0.0.1:80"},
		{URL: "http://10.0.0.2:80", OwnerEdgeEndpoint: "http://peer:8000"},
	}
	remote := []router.Backend{
		{URL: "http://10.0.0.3:80", OwnerEdgeEndpoint: "http://peer:8000"},
	}
	out := make([]router.Backend, 0)
	for _, b := range existing {
		if b.OwnerEdgeEndpoint == "http://peer:8000" {
			continue
		}
		out = append(out, b)
	}
