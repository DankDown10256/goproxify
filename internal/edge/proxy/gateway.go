...
// PeerInfo décrit une passerelle pair pour le gateway LB cross-passerelle.
...
// PeerRegistry stocke les credentials des passerelles pairs (poussés par Admin).
...
// Si OwnerEdgeEndpoint est défini, passe par le gateway de la passerelle propriétaire.
func DialBackend(ctx context.Context, b *router.Backend, peers *PeerRegistry, timeout time.Duration) (net.Conn, error) {
	...
	peer, ok := peers.LookupByEndpoint(b.OwnerEdgeEndpoint)
	if !ok || peer.Token == "" {
		return nil, fmt.Errorf("gateway: peer inconnu pour %s", b.OwnerEdgeEndpoint)
	}
	return dialGateway(ctx, peer.Endpoint, peer.Token, hostPort, timeout)
}

func dialGateway(ctx context.Context, edgeEndpoint, token, target string, timeout time.Duration) (net.Conn, error) {
	u, err := url.Parse(edgeEndpoint)
	if err != nil {
		return nil, err
	}
	addr := u.Host
