...
// Admin et Agent initialisent des connexions WS persistantes vers passerelle :8000.
// Passerelle ne contacte jamais Admin ni Agent de manière sortante.
package ws
...
type Hub struct {
	...
	adminSecret string // clé HMAC Admin→Passerelle (vide = authentification désactivée)
	...
	// Persistance des HMACs approuvés (survit aux redémarrages de la passerelle)
...
// (ex: révoquer le token dans le tokenStore local de la passerelle).
...
// SetAdminMessageHandler configure le handler pour les messages Admin→Passerelle.
func (h *Hub) SetAdminMessageHandler(fn MessageHandler) { h.onAdminMsg = fn }

// SetAgentMessageHandler configure le handler pour les messages Agent→Passerelle.
...
func (h *Hub) ApproveAgent(agentID string) error {
	...
	// Persister le HMAC pour que l'Agent puisse se reconnecter après un redémarrage de la passerelle.
