...
func fakeEdge(t *testing.T, ids []string) (*httptest.Server, *[]string) {
	t.Helper()
	var mu sync.Mutex
	published := []string{}
...
func TestSyncMissingPublishesOnlyAbsentProxies(t *testing.T) {
	rejoining, published := fakeEdge(t, []string{"p1"})
	peerA, _ := fakeEdge(t, []string{"p1", "p2", "p3"})
	peerB, _ := fakeEdge(t, []string{"p1", "p2"})

	c := NewClient()
	target := Target{NodeName: "edge-c", Endpoint: rejoining.URL}
	peers := []Target{
		target,
		{NodeName: "edge-a", Endpoint: peerA.URL},
		{NodeName: "edge-b", Endpoint: peerB.URL},
	}
...
func TestSyncMissingNoopWhenUpToDate(t *testing.T) {
	rejoining, published := fakeEdge(t, []string{"p1", "p2"})
	peer, _ := fakeEdge(t, []string{"p1", "p2"})
	c := NewClient()
...
func TestSyncMissingSurvivesUnreachablePeer(t *testing.T) {
	rejoining, published := fakeEdge(t, nil)
	good, _ := fakeEdge(t, []string{"p1"})
