...
func TestCoreEndpointsKeepsOnlyReachableCores(t *testing.T) {
	s := New(t.TempDir())
	for _, n := range []NodeEntry{
		{ID: "c1", Role: "core", Name: "frontal", Endpoint: "http://goproxify-core:8000"},
		{ID: "c2", Role: "core", Name: "backup", Endpoint: "http://192.0.2.20:8000"},
		{ID: "c3", Role: "core", Name: "declared-only"},
...
func TestCoreEndpointsEmptyWithoutFile(t *testing.T) {
	got, err := New(t.TempDir()).CoreEndpoints()
...
func TestStoreReadsHandWrittenArchitectureJSON(t *testing.T) {
  ...
  "nodes": [
    {"id": "c2", "role": "core", "name": "backup", "endpoint": "http://192.0.2.20:8000"},
    {"id": "d1", "role": "core", "name": "declared", "config": {"host": "192.0.2.30", "ha": true}}
  ]
}`
	if err := os.WriteFile(s.Path(), []byte(raw), 0o600); err != nil {
		t.Fatal(err)
	}
	eps, err := s.CoreEndpoints()
	...
	if err := s.Upsert(NodeEntry{ID: "c3", Role: "core", Name: "extra", Endpoint: "http://192.0.2.40:8000"}); err != nil {
		t.Fatal(err)
	}
