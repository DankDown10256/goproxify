...
window.applySecurityHeaderToCfg = function(cfg, name, value) {
  ...
  const stripLegacy = (k) => {
    ...
    if ((manip.forwarded_headers || []).length || manip.response_add_header) out.headers_manipulation = manip;
...
window.migrateHeadersConfig = function(cfg) {
  ...
  if (out.headers_manipulation) {
    ...
    if ((manip.forwarded_headers || []).length) out.headers_manipulation = manip;
    else delete out.headers_manipulation;
  }
  return out;
};
